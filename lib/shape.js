 ...
 *   * varMatDecStr: The declare string of only the part of Mat type.
 ...
 *   * dataDtor: The parsed, by parseDataDtor(), data descriptor object.
 *   * dimDtorArr: An array of parsed, by parseDimDtor(), dimension descriptor objects.
 *   * matDimDtorArr: An sub-array of dimDtorArr, which only contains dimension descriptors of Mat dimensions.
 *
 * For examples:
 *
 * |       Vector of Mat       |
  ...
  switch (result.type) {
      ...
      case MAT:
          ...
          if (ctype === 'Matx' || ctype === 'Vec') {
            ...
            if (result.matDims > 2 && ctype === 'Matx') {
              throw new Error('Invalid shape format: the maximum dimension size of Matx type is 2');
            }
...
function parseDataDtor(str) {
  ...
  if (str.startsWith('CV')) {
    ...
    if (parsed.ctype !== 'Mat' && parsed.channels > 1) {
      throw new Error(`Invalid data descriptor format: static Mat type (e.g., Matx, Vec) does not support multichannels`);
    }
    ...
    if (parsed.ctype === 'Matx') {
      ...
      if (parsed.channels > 1) {
        parsed.accessor = function(argusStr) {
          return `.at<Vec<${this.dtype}, ${this.channels}>>(${argusStr})`;
        }.bind({ dtype: types.cvToStd(parsed.dtype), channels: parsed.channels });
      } else {
        parsed.accessor = function(argusStr) {
