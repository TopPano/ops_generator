...
const registerOpInputFn = function() {
  return `.Input("${changeCase.snakeCase(this.name)}: ${this.type}")`;
};
...
const registerOpOutputFn = function() {
  return `.Output("${changeCase.snakeCase(this.name)}: ${this.type}")`;
};
...
const computeInputFn = function() {
  const convertTensorToCvByShapeStr = Mustache.render(convertTensorToCvByShape(this.pShape), { name: this.name });
  const dtype = this.pShape.dataDtor.format === 'cv' ? types.cvToStd(this.pShape.dataDtor.type) :
                                                       this.pShape.dataDtor.type;

  return `
  const Tensor& ${this.name} = context->input(${this.regIdx});
  OP_REQUIRES(context, ${this.name}.dims() == ${this.pShape.tfRank},
              errors::InvalidArgument("${this.name} must be ${this.pShape.tfRank}-dimensional",
              ${this.name}.shape().DebugString()));
  auto ${this.name}_data = ${this.name}.tensor<${dtype}, ${this.pShape.tfRank}>();
  ${convertTensorToCvByShapeStr}
  `;
  ...
  function convertTensorToCvByShape(shape) {
    ...
    const tensorDimSizeStr = declareDimSize(tfRank, { tensorVar, dimSizeVar });
    ...
    if (lastVecIdx !== -1) {
       ...
       * Case: Just Mat
       */
      declareStr = `const int ${tensorVar}_shape[] = { ${utils.expandArgus(dimVar, cvRank)} };
      Mat ${cvVar}(${cvRank}, ${tensorVar}_shape, ${shape[shape.length - 1]});`;
      loopStr = `${loopMat(0, cvRank, dataDtor, { cvVar, dimVar, dimSizeVar, dataVar })}`;
    ...
    function declareDimSize(rank) {
      return lodash.range(rank).map((idx) => {
        return `const int32 ${dimSizeVar}${idx} = static_cast<int32>(${tensorVar}.dim_size(${idx}));`;
      }).join('\n');
    }
    ...
    function loopVecOfCv(start, end, dataDtor) {
      if (start === end) {
        const l = utils.expandArgus((channelIdx) => {
          return `${dataVar}(${utils.expandArgus(dimVar, end)}, ${channelIdx})`;
        }, dataDtor.channels);
        const data = `Vec<${types.cvToStd(dataDtor.type)}, ${dataDtor.channels}>(${l})`;
        return `if (isnan(${dataVar}(${utils.expandArgus(dimVar, end) + ', 0'}))) { break; }
    ...
    function loopVecOfPrim(start, end) {
      if (start === end) {
        let guardStr = '';
        if (end > 1) {
          guardStr = `if (isnan(${dataVar}(${utils.expandArgus(dimVar, end) + ', 0'}))) { break; }`;
        }
        const data = `${dataVar}(${utils.expandArgus(dimVar, end)})`;
        return `${guardStr}
        ${cvVar}${utils.expandArrayAccessor(end - 1, dimVar)}.push_back(${data});`;
      }
    ...
    function loopVecOfMat(start, vecRank, matRank, dataDtor) {
      if (start === vecRank) {
        let guardStr = '';
        if (vecRank > 1) {
          guardStr = `if (isnan(${dataVar}(${utils.expandArgus(dimVar, vecRank) + ', 0'}))) { break; }`;
        }
        const data = `${dataVar}(${utils.expandArgus(dimVar, cvRank)})`;
        const tempVar = 'mat';
    ...
    function loopMat(start, end, dataDtor, cvStartOffset) {
      if (start === end) {
        ...
        if (dataDtor.channels === 1) {
          return `${cvVar}.at<${types.cvToStd(dataDtor.type)}>(${cvArgus}) = ${dataVar}(${dataArgus});`;
        } else if (dataDtor.channels > 1) {
          return lodash.range(dataDtor.channels).map((channelIdx) => {
            return `${cvVar}.at<Vec<${types.cvToStd(dataDtor.type)}, ${dataDtor.channels}>>(${cvArgus})[${channelIdx}] = ${dataVar}(${dataArgus}, ${channelIdx});`;
          }).join('\n');
        } else {
...
const computeExecuteFn = function() {
  ...
  if (this.inputoutputs) { argus = argus.concat(addPostfixToName(this.inputoutputs, '_cv')); }
...
const computeOutputFn = function() {
  ...
  const tfShapeArgusStr = Mustache.render(tfShapeArgus, { name: this.name });
  const dtype = this.pShape.dataDtor.format === 'cv' ? types.cvToStd(this.pShape.dataDtor.type) : this.pShape.dataDtor.type;
  const convertCvToTensorByShapeStr = Mustache.render(convertCvToTensorByShape(this.pShape), { name: this.name });

  return `
  Tensor *${this.name};
  OP_REQUIRES_OK(context, context->allocate_output(${this.regIdx}, TensorShape({ ${tfShapeArgusStr} }), &${this.name}));
  auto ${this.name}_data = ${this.name}->tensor<${dtype}, ${this.pShape.tfRank}>();
  ${convertCvToTensorByShapeStr}
  `;
  ...
  function loop(start, end, dataDtor) {
    if (start === end) {
      ...
      if (dataDtor.channels === 1) {
        return `${dataVar}(${utils.expandArgus(dimVar, end)}) = ${cvVar}.at<${dtype}>(${utils.expandArgus(dimVar, end)});`;
      } else if (dataDtor.channels > 1) {
        return lodash.range(dataDtor.channels).map((channelIdx) => {
          return `${dataVar}(${utils.expandArgus(dimVar, end)}, ${channelIdx}) = ${cvVar}.at<${dtype}>(${utils.expandArgus(dimVar, end)}, ${channelIdx});`;
        }).join('\n');
      } else {
        throw new Error(`Invalid channel number: ${dataDtor.channels}`);
      }
...
function render(url, callback) {
  try {
    ...
    opsMeta.opName = changeCase.upperCaseFirst(changeCase.camelCase(parsed.name));
    opsMeta.fnName = changeCase.snakeCase(parsed.name);
