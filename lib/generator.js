  ...
  function convertTensorToCvByShape(shape) {
    ...
    if (lastVecIdx !== -1) {
      ...
      if (vecLayers === cvRank) {
        ...
        declareStr = `${declareType} ${cvVar}(${dimSizeVar + "0"});`;
        loopStr = `int ${cvShapeVar}[] = { ${utils.expandArgus(dimSizeVar, vecRank, cvRank)} };
    ...
    function loopVecOfMat(start, vecRank, matRank, dataDtor) {
      if (start === vecRank) {
        ...
        return `${guardStr}
        Mat ${tempVar}(${matRank}, ${cvShapeVar}, ${type});
        ${loopMat(vecRank, matRank + vecRank, dataDtor, vecRank, tempVar)}
