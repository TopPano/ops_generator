  ...
  function convertTensorToCvByShape(pShape) {
    ...
    switch (pShape.type) {
      ...
      case parser.VEC_OF_PRIM:
        declareStr = `${pShape.varDecStr} ${cvVar}${vecInitStr};`;
      ...
      case parser.VEC_OF_MAT:
        if (pShape.dataDtor.ctype === 'Matx' ||
          ...
          ${pShape.varDecStr} ${cvVar}${vecInitStr};`;
        }
      ...
      if (start === end) {
        ...
        if (dataDtor.channels === 1) {
          ...
          return lodash.range(dataDtor.channels).map((channelIdx) => {
            // TODO: re-write this special case: .at<Vec<dtype>>(idx0, idx1, ...)[cidx0]
            return `${newCvVar ? newCvVar : cvVar}.at<Vec<${types.cvToStd(dataDtor.dtype)}, ${dataDtor.channels}>>(${cvArgus})[${channelIdx}] = ${dataInVar}(${dataArgus}, ${channelIdx});`;
          }).join('\n');
        } else {
    ...
    if (Array.isArray(opsMeta)) {
      async.each(opsMeta, (meta, callback) => {
        ...
        fs.writeFile(`${parsed.dir}/${lowerAndSnake(meta.opName)}_op.cc`,
                     _render(meta), callback);
      }, (err) => {
        if (err) { callback(err); }
      });
    } else {
      opsMeta.srcFile= opsMeta.srcFile? opsMeta.srcFile : defaultName;
      opsMeta.opName = opsMeta.opName ? capitalAndCamel(opsMeta.opName) : capitalAndCamel(defaultName);
      opsMeta.fnName = opsMeta.fnName ? opsMeta.fnName : defaultName;
      fs.writeFile(`${parsed.dir}/${lowerAndSnake(opsMeta.opName)}_op.cc`,
                   _render(opsMeta), callback);
    }
  } catch (err) {
    callback(err);
  }
